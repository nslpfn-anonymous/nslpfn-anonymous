...
class BarDistribution(nn.Module):
    ...
    def map_to_y(self, bucket_idx):
        return self.borders[bucket_idx]
    ...
    def sample(self, logits, num_samples=100):
        ...
        bucket_means = self.borders[:-1] + self.bucket_widths/2        
        return bucket_means[bucket_idx]

    def custom_sample(self, logits, target_p=None, num_samples=100):
        if target_p is None:
            num_outputs = logits.shape[-1]
            target_p = torch.rand(num_samples).to(logits.device)
            target_p = target_p[:, None, None, None]
        
        bucket_means = self.borders[:-1] + self.bucket_widths/2       
        reverse_order = [ _ for _ in range(num_outputs) ][::-1] 
        
        p = torch.nn.functional.softmax(logits, dim=-1) # M, N, num_outputs
        p = torch.cumsum(p, dim=-1) # M, N, num_outputs        
        p = p[None, :, :, :].repeat(num_samples, 1, 1, 1) # num_samples, M, N, num_outputs

        bucket_idx = (p < target_p)[:, :, :, reverse_order].float().argmax(dim=-1) # num_samples, M, N
        bucket_idx = logits.shape[-1] - 1 - bucket_idx 
        sampled_graphs = bucket_means[bucket_idx]

        """
        sampled_graphs = []        
        for idx in range(num_samples):
            temp = (p < target_p[idx].item())[:, :, reverse_order]
            bucket_idx = temp.float().argmax(dim=-1) # M, N
            bucket_idx = logits.shape[-1] - 1 - bucket_idx 
            sampled_graphs.append(bucket_means[bucket_idx])
        sampled_graphs = torch.stack(sampled_graphs, dim=0) # num_mc_samples, M, N
        """
    ...
    def icdf(self, logits, left_prob):
        ...
        if type(logits) == list:
            ...
            probs = torch.mean(bucket_probs, dim=0)
        else:
...
class FullSupportBarDistribution(BarDistribution):
    ...
    def forward(self, logits, y): # gives the negative log density (the _loss_), y: T x B, logits: T x B x self.num_bars
        ...
        if type(logits) == list:
            logits = torch.stack(logits, dim=0)
            bucket_probs = torch.softmax(logits, dim=-1)
            bucket_probs = torch.mean(bucket_probs, dim=0)
            bucket_log_probs = torch.log(bucket_probs)
        else:            
    ...
    def sample(self, logits, num_samples=100):
        ...
        for b_idx in bucket_idx:
            bucket_values = self.borders[:-1] + self.bucket_widths*torch.rand_like(self.bucket_widths)              
            bucket_values[0] = -side_normals[0].mean + self.borders[1]
            bucket_values[-1] = side_normals[1].mean + self.borders[-2]
...
def get_bucket_limits(
    ...
    if ys is not None:
        ...
        print(
            f"Using {len(ys[: -(len(ys) % num_outputs)])} y evals to estimate {num_outputs} buckets. Cut off the last {len(ys) % num_outputs} ys."
        )
