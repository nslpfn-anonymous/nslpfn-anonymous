...
class PriorDataLoader(DataLoader, metaclass=ABCMeta):
    @abstractmethod
    def __init__(self, num_steps, batch_size, **kwargs):
        """
...
class PriorDL(PriorDataLoader):
    def __init__(self, batch_func, num_steps, batch_size, **kwargs):
        super(PriorDL, self).__init__(num_steps, batch_size, **kwargs)
    ...
    def __iter__(self):
        return iter(self.batch_func(batch_size=self.batch_size) for _ in range(self.num_steps))
